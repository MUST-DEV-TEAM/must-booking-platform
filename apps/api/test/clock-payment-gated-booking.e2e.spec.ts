...
describe.skipIf(!hasSandboxCredentials)('Clock payment-gated booking (Task 4, real sandbox)', () => {
  ...
  beforeAll(async () => {
    ...
    await request(app.getHttpServer())
      .post(`${tenantUrl}/properties/${propertyId}/clock-catalog/mappings/${dblMapping.id}/confirm`)
      .set('Cookie', cookie)
      .expect(201);
  ...
  it('pay-at-hotel: creates a real Clock reservation immediately, on the same local row, then cancels it for real', async () => {
    const created = await request(app!.getHttpServer())
      ...
      .send({
        ...
        startsOn: '2026-08-16',
        endsOn: '2026-08-18',
        guest: { email: 'pay-at-hotel-guest@example.test', firstName: 'PayAtHotel', lastName: 'Guest' },
      })
      .expect(201);
  ...
  it('self-service cancellation window: blocks a near-future booking and leaves the real Clock reservation untouched', async () => {
    ...
    const created = await request(app!.getHttpServer())
      ...
      .send({
        ...
        startsOn: '2026-08-16',
        endsOn: '2026-08-18',
        guest: { email: 'window-guard-guest@example.test', firstName: 'Window', lastName: 'Guard' },
      })
      .expect(201);
    expect(created.body.ok).toBe(true);
    const bookingId = created.body.value.id as string;
    ...
    expect(cancelResult).toMatchObject({ ok: false, error: { code: 'CANCELLATION_WINDOW_CLOSED' } });
  ...
  it('online payment: continueAfterPayment (the real webhook-driven path) attaches a real Clock reservation', async () => {
    ...
    }>(clockCredentials, { api: 'pms_api', method: 'GET', path: `/bookings/${externalBookingId}` });
    expect(clockBooking.status).toBe(200);
    ...
    expect(clockBookingBody.active_notes.some((n) => n.text.includes('Deposit received: 500.00 EUR'))).toBe(
      true,
    );

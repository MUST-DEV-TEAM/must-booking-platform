...
describe('LocalPmsProvider guest matching', () => {
  ...
  it('creates a new guest and stores the phone candidate instead of attaching to it', async () => {
    ...
    ).resolves.toEqual({ ok: true, value: 'new-guest' });
  ...
  it('does not perform a phone lookup when no phone was supplied', async () => {
    ...
    await expect(resolveGuest(tx, 'tenant-1', { ...baseGuest, phone: null })).resolves.toEqual({
      ok: true,
      value: 'new-guest',
    });

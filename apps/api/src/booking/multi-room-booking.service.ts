...
export class MultiRoomBookingService {
      ...
      return await this.database.withTenantTransaction(
        ...
        async (tx) => {
          ...
          return this.withIdempotency(tx, context, command, async () => {
            const paymentMethod = await this.paymentMethod(tx, context, command.paymentMethod, command.rooms);
  ...
  private validateCommand(command: MultiRoomBookingCommand): Result<never> | null {
    if (!command.idempotencyKey || !command.quoteSessionId || !this.validDateRange(command.startsOn, command.endsOn))
    ...
    if (command.externalReference && (command.externalReference.length > 185 || !command.externalReference.trim()))
      return this.failure('INVALID_EXTERNAL_REFERENCE', 'The order reference is invalid.');
    if (!command.guest.email.trim()) return this.failure('INVALID_GUEST_EMAIL', 'Guest email is required.');
    ...
    for (const room of command.rooms) {
      if (
        ...
        (room.guestCount !== undefined && (!Number.isInteger(room.guestCount) || room.guestCount < 1))
      )
        return this.failure('MULTI_ROOM_REQUEST_INVALID', 'Each requested room needs a valid quote and total.');
    }
    return null;
  }
  ...
  private async lockInventory(
    ...
    const roomTypeIds = [...new Set(rooms.filter((room) => !room.roomId).map((room) => room.roomTypeId))].sort();
  ...
  private async reserveAll(
    ...
    for (const room of command.rooms.filter((candidate) => candidate.roomId).sort((a, b) => a.roomId!.localeCompare(b.roomId!))) {
      const reserved = await this.availability.reserveRoom(tx, context.tenantId, context.propertyId, {
        roomId: room.roomId!,
        startsOn: command.startsOn,
        endsOn: command.endsOn,
      });
    ...
    for (const [roomTypeId, units] of [...unitsByType.entries()].sort(([left], [right]) => left.localeCompare(right))) {
      const reserved = await this.availability.reserveBookedUnits(tx, context.tenantId, context.propertyId, {
        roomTypeId,
        startsOn: command.startsOn,
        endsOn: command.endsOn,
        units,
  ...
  private async paymentMethod(
    ...
    if (!selected) return this.failure('PAYMENT_METHOD_REQUIRED', 'Select an enabled payment method.');
    ...
    if (!enabled) return this.failure('PAYMENT_METHOD_NOT_ENABLED', 'The selected payment method is not enabled.');
    return {
      ok: true,
      value:
        selected === 'stripe'
  ...
  private async withIdempotency(
    ...
    if (!inserted[0]) {
      ...
      if (!operation || operation.requestHash !== requestHash)
        return this.failure('IDEMPOTENCY_KEY_CONFLICT', 'Idempotency key was used for another request.');
      return operation.result ?? this.failure('IDEMPOTENCY_IN_PROGRESS', 'Booking order is in progress.', true);
    }
  ...
  private validMoney(money: Money): boolean {
    return !!money && /^[0-9]+(?:\.\d{1,2})?$/.test(money.amount) && /^[A-Z]{3}$/.test(money.currency);
  }

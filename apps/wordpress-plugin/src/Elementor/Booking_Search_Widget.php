        ...
        <div class="must-hotel-booking-widget must-hotel-booking-widget-booking-search" data-linked-room-list-id="<?php echo \esc_attr($linkedId); ?>" data-connection-key="<?php echo \esc_attr($legacyKey); ?>">
            <form class="must-hotel-booking-booking-search" method="get" action="<?php echo \esc_url($bookingUrl); ?>" data-must-hotel-booking-mode="plugin_checkout">
                <div class="must-hotel-booking-booking-search-fields">
                    <?php foreach ([['checkin', 'Check In Date', 'calendar'], ['checkout', 'Check Out Date', 'calendar'], ['adults', 'Adults', 'people'], ['children', 'Children', 'people']] as $field) : $id = $widgetId . '-' . $field[0]; $isPartySizeField = \in_array($field[0], ['adults', 'children'], true); ?>
                        <div class="must-hotel-booking-field must-hotel-booking-field-<?php echo \esc_attr($field[0]); ?><?php echo $isPartySizeField ? ' must-hotel-booking-field-party-size' : ' must-hotel-booking-field-date'; ?>">
                            <label class="screen-reader-text" for="<?php echo \esc_attr($id); ?>"><?php echo \esc_html__($field[1], 'must-hotel-booking'); ?></label>
                            <input id="<?php echo \esc_attr($id); ?>" type="<?php echo $isPartySizeField ? 'number' : 'text'; ?>" name="<?php echo \esc_attr($field[0]); ?>" class="must-hotel-booking-<?php echo $isPartySizeField ? 'party-size must-hotel-booking-' . $field[0] : 'date-input must-hotel-booking-' . $field[0]; ?>" placeholder="<?php echo \esc_attr__($field[1], 'must-hotel-booking'); ?>" autocomplete="off" <?php echo $isPartySizeField ? ($field[0] === 'adults' ? 'min="1"' : 'min="0"') . ' max="' . \esc_attr((string) MustBookingConfig::get_max_booking_guests()) . '" step="1" inputmode="numeric" pattern="[0-9]*"' : 'required'; ?> />
                            <img class="must-hotel-booking-field-icon" src="<?php echo \esc_url($icons[$field[2]]); ?>" alt="" aria-hidden="true" />
                        </div>
                    <?php endforeach; ?>
                    <input type="hidden" name="guests" value="1" class="must-hotel-booking-guests" />
                </div>

...
- Milestone 21 Task 17 adds a selected-range search advisory through MUST's public availability-check endpoint. It calls the shared `ClockAvailabilityService.isAvailableForBooking` path with a 45-second tenant/property/room/date-range cache; provider failures fail open for search, while the existing uncached pre-payment guard remains authoritative. Calendar month/day rendering does not call Clock for this check.

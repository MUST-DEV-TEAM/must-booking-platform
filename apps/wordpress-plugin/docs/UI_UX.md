...
- A physical room may optionally supply its own display title and free-text size. The title replaces the detail-page heading only when set; an unset size is omitted from the existing metadata row.
- A room type may supply optional amenities-introduction copy. The public catalog exposes it once per room type, and the room-details page renders it above that type's effective amenity grid.
- A property can supply default room rules, with an optional physical-room override that fully replaces it. The public catalog resolves the effective text; the room-details page always renders the Room Rules section, using its approved empty-state copy when no effective text exists.
- Physical rooms inherit their room type's amenities until staff assign room-specific amenities. The public catalog resolves the effective set, and the room-details page renders that set without applying its own fallback.
- The room-details page may render an optional amenities introduction separately from the structured amenity grid. Its inquiry CTA uses the WordPress site admin email as a `mailto:` fallback, and its terms CTA is shown only when a published WordPress terms page can be resolved from a common terms slug.

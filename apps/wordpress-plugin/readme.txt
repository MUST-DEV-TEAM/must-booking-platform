...
= 0.4.97 =
* Compact accommodation filter titles and buttons, align the guest summary with the confirmation fields, and remove the checkout Special Requests outline/glow.

= 0.4.96 =
* Fixed release packaging so the required asset URL helper is always included and validated before publication.

= 0.4.95 =
* Fixed release updater version detection when the distribution repository's source metadata is stale.
* Release tags now carry the exact plugin metadata used by WordPress while the attached ZIP remains canonical.

= 0.4.94 =
* Restored the total-guests booking-search control.
* Improved accommodation filter-panel presentation.
* Replaced room-card zero-price placeholders with provider-backed display totals or an unavailable state.
* Added guest-composition text after the confirmation email field.
* Removed checkout contact-field outlines and focus glow.

= 0.4.93 =
* Added adult/child party controls, capacity validation, and confirmation occupancy details.

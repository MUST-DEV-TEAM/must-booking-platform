...
async function fail(source, message) {
  try {
    ...
    console.error(
      `CRITICAL: ${message} (and reporting to Sentry failed: ${error instanceof Error ? error.message : String(error)}).`,
    );
  }
  process.exitCode = 1;
}
...
if (typeof process.getuid === 'function' && process.getuid() === 0) {
  await fail(
    'deploy-preflight',
    'Deploy drift check was invoked as root. It must run as the dedicated deploy user.',
  );
} else {
  try {
    if (process.platform === 'linux' && typeof process.getuid === 'function') {
      ...
      if (foreignPath) {
        await fail(
          'deploy-preflight',
          `Deploy checkout ownership drift: ${foreignPath} is not owned by the deploy user.`,
        );
      } else {
    ...
    if (!process.exitCode) {
      ...
      if (deployedCommit !== originCommit)
        await fail(
          'deploy-drift',
          `Deploy drift detected: deployed=${deployedCommit} origin/main=${originCommit}`,
        );
    ...
    await fail(
      'deploy-drift',
      'Deploy drift check could not fetch origin/main as the deploy user.',
    );
  }
}

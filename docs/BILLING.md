...
- **Hard limits** — properties and manually invited staff seats per tenant: the action is **blocked** once the plan's cap is reached. The three generic staff accounts automatically provisioned for every new property (Front Desk, Property Manager, Finance) are cap-exempt so property creation remains possible on every plan; they are still ordinary, login-capable `STAFF` memberships that an admin may rename, reassign, or remove.

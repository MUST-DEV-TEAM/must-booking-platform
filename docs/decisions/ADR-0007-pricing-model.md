...
- Stripe Billing (ADR-0003) product/price catalog should mirror the *final* table (not the illustrative one above): one Stripe Product per plan tier (a free tier may be a $0 price or handled outside Stripe entirely — decide during Milestone 8).
- The real plan table is defined together with the owner when Milestone 8 (Platform Billing) starts, not assumed from the illustrative example — this is a milestone-kickoff task, not implementation guesswork.

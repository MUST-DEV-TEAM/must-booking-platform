...
## Clock's own event payload shape: unconfirmed

`ClockWebhookService.eventTypeOf`/`objectIdOf` try to parse SNS's `Message` field as JSON and read a `type`/`id` (or `object_id`) field, falling back to the SNS envelope's own `Type` if that fails. **This shape is a guess** — the source brief doesn't specify Clock's Message Channels payload format, and no live event has ever been captured (no real Clock Message Channels subscription exists for this account — see `CLOCK_ENDPOINT_MATRIX.md`). The fallback exists so a real event never gets silently mislabeled as empty; it does not mean the guessed shape is confirmed correct.
...
No live Clock Message Channels subscription exists for this account (would require Clock account-dashboard configuration outside this session's reach), so genuinely AWS-signed traffic was never captured. `clock-webhook.e2e.spec.ts` instead exercises the entire flow above for real — real HTTP routing through the actual running app, the real RLS-carve-out lookup, real RSA signature verification (against a real locally-generated key pair, not a mock), real dedup, real BullMQ enqueue, real 400/404 handling — with only the literal "fetch cert bytes from AWS" network call replaced by a stub via dependency injection (`ClockWebhookVerificationService`'s `fetchCert` constructor param, the same testability pattern `ClockHttpClient`'s `protocol` override already established for testing against a local server instead of a live third party). The SSRF-protecting hostname check itself is not stubbed and was tested against both real and fake AWS host strings.

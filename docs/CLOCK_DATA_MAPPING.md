...
Clock guest identities are stored in `clock_guest_mappings`, scoped by tenant, property, and local guest. A mapping is deliberately separate from `clock_catalog_mappings`: catalog rows are proposed/confirmed by staff, while guest identity resolution is automatic at booking time. Before either booking-create path (`attachRealReservation` or `createBooking`) sends its POST, MUST reads this mapping first. A mapped guest passes the stored `external_guest_id` as top-level `main_booking_guest` and makes zero guest-search calls. On a mapping miss, MUST makes one `/guests/search` call by email, filters the fuzzy results for an exact `e_mail` match, and persists the resolved Clock `family_id` when one is found. When email does not match, the booking keeps the inline `guest_e_mail`/`guest_first_name`/`guest_last_name` fields and lets Clock create the guest as before; if the booking response includes `main_booking_guest.guest_id`, that id is persisted for the next booking. Both local provider paths share the local email/phone matcher; external ID and merge UI remain separate concerns. Source brief section 24's fuller guest-matching requirement (external ID, email, phone, booking relationship, manual confirmation) is partially built here, with the external Clock identity now retained per property.

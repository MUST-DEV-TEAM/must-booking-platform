...
No Clock-side guest id is stored or reconciled — MUST's guest record and Clock's guest record remain independent. Before either booking-create path (`attachRealReservation` or `createBooking`) sends its POST, MUST makes one `/guests/search` call by email, filters the fuzzy results for an exact `e_mail` match, and passes top-level `main_booking_guest` when it identifies a Clock `family_id`. When email does not match, the booking keeps the inline `guest_e_mail`/`guest_first_name`/`guest_last_name` fields and lets Clock create the guest as before. The resolved `family_id` is not persisted. Both local provider paths share the local email/phone matcher; external ID and merge UI remain deferred. Source brief section 24's fuller guest-matching requirement (external ID, phone, booking relationship, manual confirmation) is only partially built here.
